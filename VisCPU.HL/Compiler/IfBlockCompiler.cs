...
namespace VisCPU.HL.Compiler
{
    public class IfBlockCompiler : HLExpressionCompiler<HLIfOp>
    {
        ...
        public override ExpressionTarget ParseExpression(
            HLCompilation compilation, HLIfOp expr)
        {
            ...
            string iftempVar = compilation.GetTempVar("if_tmp");
            for (int i = 0; i < expr.ConditionMap.Count; i++)
            {
                ...
                ExpressionTarget exprTarget = compilation.Parse(expr.ConditionMap[i].Item1, new ExpressionTarget(iftempVar, true));
                string nextLabel;
