...
namespace VisCPU.HL.Compiler.Special
{
    ...
    public class ForExpressionCompiler : HLExpressionCompiler<HLForOp>
    {
        ...
        public override ExpressionTarget ParseExpression(HLCompilation compilation, HLForOp expr)
        {
            ...
            compilation.EmitterResult.Store($".{startLabel} linker:hide");                     //Unused, makes clear where the for compiler started emitting code.

            ExpressionTarget target = compilation.Parse(expr.VDecl).MakeAddress(compilation); //Declare Variable(left)
            ...
            compilation.EmitterResult.Emit($"BEZ", cond.ResultAddress, endLabel); //Check if Expression "Equal to Zero" => jump to end if it is

            foreach (HLExpression hlExpression in expr.ExprBody)
            {
                compilation.ReleaseTempVar(compilation.Parse(hlExpression).ResultAddress); //Parse block and clean up any temp variables that were emitted.
            }
            ...
            compilation.EmitterResult.Store($".{endLabel} linker:hide"); //End label that we jump to if we exit the loop
